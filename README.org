...
1. Modify [[./modules/users/_keys.nix]] to add your user and public key following
   the existing format.
2. If you want to add a new secret, edit [[./secrets/secrets.nix]] and include it
   there. Use ~everyone~ as the list of public keys.
...
To build and run the bootstrap VM locally for testing:

#+begin_src shell
  # Build the bootstrap VM image (stage 1 — minimal config)
  just build-qemu                       # or: just bq

  # Build and run in one step
  just run-qemu                         # or: just rq
#+end_src

By default this targets the ~bootstrap~ stage. To launch the full ~nekoma~ stage
(which includes PostgreSQL, Lyceum, secrets):

#+begin_src shell
  TARGET_VM=nekoma just run-qemu
#+end_src

The VM exposes the following ports on the host via QEMU port forwarding:

| Host Port | Guest Port | Service                  |
|-----------+------------+--------------------------|
|      2222 |         22 | SSH                      |
|      4369 |       4369 | EPMD (Erlang)            |
|      9100 |       9100 | Erlang Distribution Port |

Inside the VM, verify services are running correctly:

#+begin_src shell
  systemctl status sshd.service
  # or, if running the nekoma stage
  sudo --user postgres psql
#+end_src

You can also add a convenient SSH config entry (in ~$HOME/.ssh/config~):
...
Once the bootstrap VM is up, you can deploy the full nekoma configuration:
